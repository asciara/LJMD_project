...
void force(mdsys_t *sys) 
{
    ...
    for(i=sys->mpirank; i < sys->natoms-1; i+=sys->nsize) {
		...
		//The c array contains, for every particle in the box, the sum of force contributions coming from particles in a given rank. We need to sum (MPI_Reduce) to sum the 
